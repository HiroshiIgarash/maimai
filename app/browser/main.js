...
const searchRealLevel = (document, musicTitle, isDx, rank) => {
  ...
  const reg = isDx
    ? new RegExp(`^${escapedMusicTitle}\\[dx`)
    : new RegExp(`^${escapedMusicTitle}$`);

  const targetMusicElm = Array.from(musics).filter((m) =>
    m.innerHTML.match(reg)
  );
...
const generateMusicScoreObjFromPlayInfo = (playInfo, tableDocument) => {
  playInfo.map((info) => {
    ...
    const { lv: realLevel, isNew } = searchRealLevel(
      tableDocument,
      info.name,
      info.isDx,
      "m"
...
const init = async () => {
  const levelTableDocument = await fetchHTMLDocument(
    "http://192.168.68.84:5500/table.html"
  );

  const playInfo = generatePlayInfoFromDoc();

  const musicScoreObj = generateMusicScoreObjFromPlayInfo(
    playInfo,
    levelTableDocument
  );
