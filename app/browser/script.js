...
const generatePlayInfoFromDoc = (doc = window?.document) => {
  ...
  const musicInfoObj = Array.from(musicInfoElms)
    .map((infoElm) => {
      const musicTitle = infoElm
        .querySelector(".music_name_block")
        .lastChild.textContent.trim();
      const scorePercent = infoElm
        .querySelector(".music_score_block")
        ?.innerHTML.replace("%", "");
      const scoreNum = scorePercent ? Number(scorePercent) / 100 : undefined;
      const displayedLevelStr =
        infoElm.querySelector(".music_lv_block").innerHTML;
      const displayedLevelNum = Number(displayedLevelStr.replace("+", ".5"));
      const isDx =
        infoElm.querySelector(".music_kind_icon").src ===
...
const generatePlayCountFromDoc = (doc = window?.document) => {
  ...
  const musicInfoObj = Array.from(musicInfoElms).map((infoElm) => {
    const musicTitle = infoElm
      .querySelector(".music_name_block")
      .lastChild.textContent.trim();
    const playCount = infoElm
      .querySelector(".music_score_block")
      ?.innerText.match(/：(.*)回/)?.[1];
    if (!playCount)
      console.log(infoElm.querySelector(".music_score_block")?.innerText);
    const isDx =
      infoElm.querySelector(".music_kind_icon").src ===
      "https://maimaidx.jp/maimai-mobile/img/music_dx.png";
 ...
 * @returns
 */
const generatePlayHistoryFromDoc = (doc = window?.document) => {
  const musicInfoElms = doc.querySelectorAll(".p_10.t_l.f_0.v_b");
  ...
  const musicInfoObj = Array.from(musicInfoElms)
    .map((infoElm) => {
      ...
      const difficultyImageSrc = infoElm.querySelector(".playlog_diff").src;
      const difficulty = (() => {
        if (difficultyImageSrc.includes("remaster")) return "r";
        if (difficultyImageSrc.includes("master")) return "m";
        return "e";
      })();
      if (!infoElm.querySelector(".playlog_music_kind_icon")) return;
      const isDx =
        infoElm.querySelector(".playlog_music_kind_icon").src ===
        "https://maimaidx.jp/maimai-mobile/img/music_dx.png";
 ...
 * @param {'e' | 'm' | 'r'} difficulty
 */
const generatePlayHistory = async () => {
  console.log(`プレイ履歴を取得中・・・`);

  const doc = await fetchHTMLDocument(
    `https://maimaidx.jp/maimai-mobile/record/`
  );
  const playHistory = generatePlayHistoryFromDoc(doc);
...
const main = async () => {
  ...
  const playHistory = await generatePlayHistory();

  // プレイ回数をplayInfoにマージ
  playCount.e.forEach((playCountItem) => {
    const { name, count, isDx } = playCountItem;
    const playInfoIndex = playInfo.e.findIndex(
      (item) => item.name === name && item.isDx === isDx
    );
    if (playInfoIndex !== -1) {
      playInfo.e[playInfoIndex].count = count;
    } else {
      console.log(
        `プレイ情報が見つかりません:e ${name} (${isDx ? "DX" : "N"})`
      );
    }
  });
  playCount.m.forEach((playCountItem) => {
    const { name, count, isDx } = playCountItem;
    const playInfoIndex = playInfo.m.findIndex(
      (item) => item.name === name && item.isDx === isDx
    );
    if (playInfoIndex !== -1) {
      ...
      console.log(
        `プレイ情報が見つかりません:m ${name} (${isDx ? "DX" : "N"})`
      );
    }
  });
  playCount.r.forEach((playCountItem) => {
    const { name, count, isDx } = playCountItem;
    const playInfoIndex = playInfo.r.findIndex(
      (item) => item.name === name && item.isDx === isDx
    );
    if (playInfoIndex !== -1) {
      ...
      console.log(
        `プレイ情報が見つかりません:r ${name} (${isDx ? "DX" : "N"})`
      );
    }
  });

  // プレイ履歴をplayInfoにマージ
  playHistory.forEach((playHistoryItem) => {
    const { name, difficulty, isDx } = playHistoryItem;
    const playInfoIndex = playInfo[difficulty].findIndex(
      (item) => item.name === name && item.isDx === isDx
    );
    if (playInfoIndex !== -1) {
      playInfo[difficulty][playInfoIndex].isPlayedRecently = true;
    } else {
      console.log(
        `プレイ情報が見つかりません:${difficulty} ${name} (${
          isDx ? "DX" : "N"
        })`
      );
    }
