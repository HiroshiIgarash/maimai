...
export const calculateGoalScore = (goalMusicRating: number, level: number) => {
  ...
  if (minRatingInRank("AAA") >= goalMusicRating && minRatingInRank("AA") < goalMusicRating) {
    return Math.min(goalMusicRating / (rateByRank("AA") * level), rankNeedScoreMap["AAA"])
  }

  // ランクAAA範囲内、もしくはランクS到達で達成する場合
  if (minRatingInRank("S") >= goalMusicRating && minRatingInRank("AAA") < goalMusicRating) {
    return Math.min(goalMusicRating / (rateByRank("AAA") * level), rankNeedScoreMap["S"])
  }

  // ランクS範囲内、もしくはランクS+到達で達成する場合
  if (minRatingInRank("S_PLUS") >= goalMusicRating && minRatingInRank("S") < goalMusicRating) {
    return Math.min(goalMusicRating / (rateByRank("S") * level), rankNeedScoreMap["S_PLUS"])
  }

  // ランクS+範囲内、もしくはランクSS到達で達成する場合
  if (minRatingInRank("SS") >= goalMusicRating && minRatingInRank("S_PLUS") < goalMusicRating) {
    return Math.min(goalMusicRating / (rateByRank("S_PLUS") * level), rankNeedScoreMap["SS"])
  }

  // ランクSS範囲内、もしくはランクSS+到達で達成する場合
  if (minRatingInRank("SS_PLUS") >= goalMusicRating && minRatingInRank("SS") < goalMusicRating) {
    return Math.min(goalMusicRating / (rateByRank("SS") * level), rankNeedScoreMap["SS_PLUS"])
  }

  // ランクSS+範囲内、もしくはランクSSS到達で達成する場合
  if (minRatingInRank("SSS") >= goalMusicRating && minRatingInRank("SS_PLUS") < goalMusicRating) {
    return Math.min(goalMusicRating / (rateByRank("SS_PLUS") * level), rankNeedScoreMap["SSS"])
  }

  // ランクSSS範囲内、もしくはランクSSS+到達で達成する場合
  if (minRatingInRank("SSS_PLUS") >= goalMusicRating && minRatingInRank("SSS") < goalMusicRating) {
    return Math.min(goalMusicRating / (rateByRank("SSS") * level), rankNeedScoreMap["SSS_PLUS"])
  }
